<master src="./master">
<property name="title">dotLRN Admin</property>
<property name="context_bar">@context_bar@</property>

<h3>Add an Instructor</h3>

<form method="get" action="add-instructor-2">
  <if @referer@ not nil><input type="hidden" name="referer" value="@referer@"></input>
  <input type="hidden" name="community_id" value="@community_id@"></input>
  Add an Instructor: <input type="text" name="search_text"><input type="submit" value="search"></input>
</form>
